...
async function addItems(li, listCont, positionsIds) {
...
async function validate(card, value) {
    let result = await query('drug', 'GET', undefined, { id: value });
    result = await result.json();
    const recipe = result[0].recipe;
    const itemNameSelect = card.querySelector('.order-recipe select');
    return recipe ? itemNameSelect.value === value : true;
}
async function validateRecipe(card) {
...
async function addItem(li, listCont, options) {
    ...
    const itemNameSelect = await addSelectSql(itemName, {
        ...
        value: options === null || options === void 0 ? void 0 : options.drug
    });
    ...
    buttonApprove.addEventListener('click', async () => {
        ...
        let result = await query('drug', 'GET', undefined, {
...
function enableDragAndDropCard(card) {
    ...
    card.addEventListener('drop', async (e) => {
        ...
        if (!(await validate(card, item.querySelector('.item-name select').value))) {
            alert('Невозможно добавить препарат, так как он требует рецепт');
            return;
        }
